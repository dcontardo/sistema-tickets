...
const App = () => {
  ...
  // Determina el color de fondo del AppBar según el tipo de usuario
  const getAppBarBg = () => {
    ...
    switch (usuario.tipo) {
      ...
      default:
        return theme.palette.primary.main; // azul para estudiante
    }
  };

  // Links del menú (sin Historial, y métricas solo si no es estudiante)
  const NavLinks = (
    <>
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      <AppBar
        position="static"
        sx={{ backgroundColor: getAppBarBg() }}
      >
        <Toolbar>
          {usuario && isMobile && (
            <>
        ...
        <Routes>
          ...
          {usuario ? (
            <>
              ...
              {/* Métricas: redirigir estudiantes a /tickets */}
              <Route
                path="/metricas"
                element={
                  usuario.tipo === "estudiante" ? (
